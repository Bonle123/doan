@if (isset($product))
    <div class="product-item">
        <a href="{{ route('get.product.detail',$product->pro_slug . '-'.$product->id ) }}" title="" class="avatar image contain">
            <img alt="{{  $product->pro_name }}" data-src="{{ pare_url_file($product->pro_avatar) }}" src="{{  asset('images/no-image.jpg') }}" class="lazyload lazy">
        </a>
        ...
        @if ($product->pro_number <= 0)
            <p style="position: absolute;right: 10px;color: #E91E63;font-weight: bold;">Sản phẩm đã hết</p>
        @endif
        <p class="rating">
            <span>
                @php 
